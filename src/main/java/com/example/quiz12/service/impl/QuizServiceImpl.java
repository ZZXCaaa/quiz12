...
public class QuizServiceImpl implements QuizService {
	...
	// 因為同時新增問卷和問題，新增多筆資料都算是同一次的行為，所以要嘛全部成功，要嘛全部失敗
	...
	public BasicRes create(CreateReq req) {
		...
		// TODO 增加新增資料失敗: 使用 try-catch，包含 @Transactional 的資料回朔層級
		try {
			...
			return new BasicRes(ResMessage.DATA_SAVE_ERRO.getCode(), //
					ResMessage.DATA_SAVE_ERRO.getMessage());
		}
	}
...
//			}
	...
	public QuizSearchRes getQuiz(SearchReq req) {
		...
		// 如果 name 沒有條件, 前湍帶過的資料可能會是 null 或是 空字串
		// 改變條件值: 如果name 是 null或是空字串或空白字串,一律替換空白字串
		String name = req.getName();
		if(!StringUtils.hasText(name))
		{
			//SQl 語法裡,like %% (兩個%中間是空字串)表示 忽略該欄位的條件示
			name = "";
		}
		LocalDate strDate =req.getStarLocalDate();
		if(strDate == null) //starDate == null 表示開始時間此欄位沒又帶值
		{
			//沒有帶值,可以把時間移到很早以前
			strDate = LocalDate.of(1970, 1, 1);
		}
		LocalDate endDate =req.getEndDate();
		if(endDate == null) //starDate == null 表示開始時間此欄位沒又帶值
		{
			//沒有帶值,可以把時間移到很久以後
			strDate = LocalDate.of(2999, 12, 31);
		}
		List<Quiz> res = quizDao.getQuiz(name, strDate, endDate);
		return new QuizSearchRes(ResMessage.SUCCESS.getCode(), ResMessage.SUCCESS.getMessage(), res);
	}
	...
	public BasicRes deleate(DeleatReq req) {
		...
		{
			return new BasicRes(ResMessage.DATA_SAVE_ERRO.getCode(),ResMessage.DATA_SAVE_ERRO.getMessage());
		}
	...
	public BasicRes Update(UpdateReq req) 
	{
		...
		//檢查問題中的 quizId 是否與 req 中的 quizId 相同
		for (Question item :req.getQuestionList() ) 
		{
			if(req.getQuizId()!= item.getQuesId())
			{
		...
		{
			return new BasicRes(ResMessage.DATA_UPDATE_ERRO.getCode(),ResMessage.DATA_UPDATE_ERRO.getMessage());
		}
